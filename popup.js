...
async function 전송() {
  ...
  const 프로필 = 현재프로필();
  // [진단 전용] 단독 실행 대조 스위치 (기본값 "" = 세 곳 모두, 기존 동작)
  const 단독대상 = (현재설정 && 현재설정.단독대상) || "";
  질문칸.value = "";
  질문칸.dispatchEvent(new Event("input"));
  const 보낼첨부 = 첨부목록;
  첨부목록 = [];
  ...
  try {
    const 응답 = await chrome.runtime.sendMessage({
      ...
      프로필ID: 프로필 ? 프로필.id : "표준",
      // 명령바에서 대상 선택을 없앴으므로 기본은 항상 세 곳 모두입니다.
      // [진단 전용] 설정에 단독대상이 지정된 경우에만 그 한 곳으로 좁힙니다.
      사이트사용: 단독대상 
        ? {
            claude: 단독대상 === "claude",
            chatgpt: 단독대상 === "chatgpt",
            gemini: 단독대상 === "gemini",
          }
        : { claude: true, chatgpt: true, gemini: true },
    });
    상태표시((응답 && 응답.결과들) || []);
