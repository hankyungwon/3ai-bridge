...
async function 창정리(짝 = null) {
  ...
  // 활성 탭 표시등(하단선)용 상태 기록.
  // 예전에는 겹침 모드에서 창을 focus 할 때 onFocusChanged 가 우연히
  // 기록해 주는 것에 기대고 있었고, 3분할 모드는 focus 호출이 없어
  // 아무 탭도 활성으로 표시되지 않았습니다. 이제 배치 경로에서 직접
  // 기록합니다: 이전 값이 아직 유효하면 유지하고, 없으면 가운데 창.
  try {
    const { 현재전면사이트 } = await chrome.storage.session.get("현재전면사이트");
    const 유효 = 현재전면사이트 && 탭지도[현재전면사이트];
    const 기준 = 유효 ? 현재전면사이트 : 순서[1];
    if (탭지도[기준]) {
      await chrome.storage.session.set({ 현재전면사이트: 기준 });
    }
  } catch (e) {
    /* 표시등 기록 실패는 배치에 영향 주지 않음 */
  }
  return 탭지도;
}
...
/**
 * 명령바에 "패널을 접으라"고 알립니다.
 * 명령바가 닫혀 있으면 받는 곳이 없어 실패하는데, 그건 정상이라 무시합니다.
 */
function 패널접기알림() {
  try {
    chrome.runtime.sendMessage({ 종류: "패널접기" }).catch(() => {});
  } catch (e) {
    /* 무시 */
  }
}
...
chrome.runtime.onMessage.addListener((메시지, 발신, 응답) => {
  ...
  if (메시지.종류 === "답변수집") {
    (async () => {
      ...
      // 걷어 온 순서는 "먼저 끝난 곳" 순서라 매번 다릅니다.
      // 사용자가 기대하는 순서(설정의 창 순서 = 제비·참새·하마)로 맞춰 돌려줍니다.
      const 정렬된결과 = {};
      for (const 키 of 설정.창순서) {
        if (결과[키]) 정렬된결과[키] = 결과[키];
      }
      for (const [키, 값] of Object.entries(결과)) {
        if (!정렬된결과[키]) 정렬된결과[키] = 값; // 창순서에 없는 곳도 빠뜨리지 않음
      }
      응답({ 결과: 정렬된결과, runId });
    })();
    return true;
  ...
  if (메시지.종류 === "창정리") {
    // 정렬은 "원래 모습으로 되돌리기"입니다. 펼쳐 둔 패널이 있으면 먼저 접어
  ...
  if (메시지.종류 === "동시질문") {
    (async () => {
      ...
      // 대상에만 동시에(병렬로) 보냅니다 — 한 곳이 실패해도 나머지는 진행됩니다.
      // 둘만 보기(짝) 중이면 대상이 두 곳이므로, 최소화해 둔 나머지 한 곳에는
      // 질문이 전달되지 않습니다(엔터·전송 버튼 모두 이 경로 하나만 씁니다).
