...
**육안 확인은 두 곳에서** — 입력창과 전송된 버블을 따로 봅니다.
입력창부터 짧으면 **주입 실패(C<B, 1순위)**, 입력창은 멀쩡한데 버블만 짧으면
**조기 전송(D<C, 2순위)** 입니다. 로그가 나오기 전에 두 가설이 갈립니다.

**2단계가 0/3 실패면** `tools/grow-payload.mjs` 로 한 번에 한 변수씩 키웁니다
(길이 2배 → 개행 2배 → 구조 2배). 재현되는 순간 멈추고 그 파일을 새 P1 으로
삼으며, 마지막에 바꾼 변수가 임계 변수입니다.
