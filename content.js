...
(() => {
  ...
  // [진단 전용] 계측 모듈이 같은 규칙으로 요소를 찾을 수 있게 노출합니다.
  // (탐색 규칙 자체는 바꾸지 않습니다)
  globalThis.__3대장_요소찾기 = 요소찾기;
   ...
   */
  async function 전송하기(설정, 입력란, 첨부있음, 본문 = "", 계측기 = null) {
    let 버튼눌림 = false;
    const 목표 = 글자만(본문);
    // [진단 전용] 아래 계측 호출들은 모두 동기·무반환이며 조건식에 관여하지 않습니다.
    const 진단 = globalThis.전송진단 || null;
    let 이전노드 = null;
    let 이전내용맞음 = null;
    ...
    for (let i = 0; i < 시도횟수; i++) {
      ...
      const 지금입력란 = 요소찾기(설정.입력란) || 입력란;
      const 실제글자 = 글자만(원문읽기(지금입력란));
      const 내용맞음 = !목표 || 실제글자 === 목표;

      // ── [진단] 경량 기록: 매 회차, 레이아웃을 건드리지 않습니다 ──
      if (진단 && 계측기) {
        진단.회차기록(계측기, {
          회차: i + 1,
          내용맞음,
          ...
          실제길이: 실제글자.length,
          버튼,
          선택자: 버튼 ? 찾은선택자(설정.전송버튼, 버튼) : null,
        });
        // ── [진단] 중량 기록: 첫 발견 / 노드 교체 / 상태 변화 때만 ──
        const 지금노드 = 버튼 ? 진단.노드번호(버튼) : null;
        if (지금노드 !== 이전노드 || 내용맞음 !== 이전내용맞음) {
          진단.체크포인트(계측기, 이전노드 === null ? "첫관측" : "상태변화", {
            선택자: 버튼 ? 찾은선택자(설정.전송버튼, 버튼) : null,
            후보통계: 후보통계내기(설정.전송버튼),
            버튼,
            입력란: 지금입력란,
      ...
      if (
        ...
        // ── [진단] 클릭 직전 상태와 전달 감시. 클릭 자체는 그대로입니다 ──
        let 감시해제 = null;
        if (진단 && 계측기) {
          진단.체크포인트(계측기, "클릭직전", {
            선택자: 찾은선택자(설정.전송버튼, 버튼),
            후보통계: 후보통계내기(설정.전송버튼),
            버튼,
            입력란: 지금입력란,
            실제글자,
          });
          감시해제 = 진단.클릭전달감시(계측기, 버튼, 지금입력란);
        }

        버튼.click();

        if (진단 && 계측기) {
          if (감시해제) 감시해제(); // 버튼·document 리스너는 즉시 해제
          진단.스냅샷예약(계측기, 설정, 지금입력란);
        }
        버튼눌림 = true;
        break;
      }
    ...
    if (!버튼눌림 && 목표) {
      ...
      if (지금글자 !== 목표) {
        return {
          성공: false,
          사유: `입력 내용이 원문과 달라 전송을 멈췄습니다 (${지금글자.length}/${목표.length}자) [SEND-CONTENT-MISMATCH]`,
          코드: "SEND-CONTENT-MISMATCH",
        };
      }
    }
    ...
    // [진단] 예전에는 (a)버튼 미발견과 (b)버튼은 있으나 계속 조건 불충족을
    // 같은 문구로 뭉갰습니다. 원인이 전혀 다르므로 갈라서 표시합니다.
    if (버튼눌림) {
      return {
        성공: false,
        사유: "전송 버튼을 눌렀지만 전송이 확인되지 않음 [SEND-CLICKED-UNCONFIRMED]",
        코드: "SEND-CLICKED-UNCONFIRMED",
      };
    }
    const 버튼있었나 = !!요소찾기(설정.전송버튼);
    if (진단 && 계측기) {
      진단.체크포인트(계측기, "최종실패", {
        선택자: null,
        후보통계: 후보통계내기(설정.전송버튼),
    ...
    return 버튼있었나
      ? {
          성공: false,
          사유:
            "전송 버튼은 있었지만 끝내 누를 수 있는 상태가 아니었습니다 [SEND-BUTTON-NEVER-READY]",
          코드: "SEND-BUTTON-NEVER-READY",
        }
      : {
          성공: false,
          사유: "전송 버튼을 찾지 못함 (selectors.js 갱신 필요) [SEND-BUTTON-NOT-FOUND]",
          코드: "SEND-BUTTON-NOT-FOUND",
        };
  }

  /* ─── [진단 전용] 보조 함수 두 개 — 읽기만 합니다 ─── */

  /** 이 버튼이 후보 목록의 몇 번째 선택자로 잡혔는지 알아냅니다. */
  function 찾은선택자(후보들, 요소) {
    try {
      for (let i = 0; i < (후보들 || []).length; i++) {
        if (요소.matches(후보들[i])) return { 선택자: 후보들[i], 순번: i };
      }
    } catch (e) {
      /* 무시 */
    }
  ...
  /** 선택자별로 후보가 몇 개이고 그중 보이는 것이 몇 개인지 셉니다. */
  function 후보통계내기(후보들) {
    try {
      return (후보들 || []).map((선택자) => {
        ...
        for (const el of 목록) {
          ...
          if (r.width > 0 && r.height > 0) 보이는수++;
        }
        return { 선택자, 개수: 목록.length, 보이는수 };
      });
    } catch (e) {
      return null;
    }
  ...
  let 전송진행중 = false; // 같은 탭에서 전송이 겹치지 않게 하는 잠금
  ...
  chrome.runtime.onMessage.addListener((메시지, _발신, 응답) => {
    ...
    // ★ 같은 탭에 전송이 겹쳐 들어오는 것을 막습니다. ★
    // 빠르게 타이핑하다 Enter 를 두 번 누르면 전송이 두 번 시작되어,
    // 한 주입이 다른 주입을 덮어쓰고 글이 입력창에 남거나 같은 질문이
    // 두 번 나가는 일이 있었습니다. 전송 로직 자체는 그대로 두고
    // 그 바깥만 감쌉니다.
    if (전송진행중) {
      응답({
        성공: false,
        사유: "앞선 전송이 아직 진행 중이라 건너뛰었습니다 [SEND-BUSY]",
        코드: "SEND-BUSY",
      });
      return true;
    }
    ...
    (async () => {
      ...
      // [진단 전용] 계측기를 켭니다. 전송 함수는 이것을 기다리지 않으며,
      // 계측기는 전송이 끝난 뒤에도 잠시 혼자 관찰하다 스스로 정리됩니다.
      const 계측기 = globalThis.전송진단
        ? globalThis.전송진단.시작(메시지.사이트, 메시지.본문 || "")
        : null;
      ...
      // 전송 함수는 여기서 곧바로 반환합니다(계측 대기 없음).
      if (globalThis.전송진단 && 계측기) {
        globalThis.전송진단.종료신호(
          계측기,
          전송결과.성공 ? "SEND-OK" : 전송결과.코드 || "SEND-UNKNOWN"
        );
        // 안전 종료: terminal 이 와도 스냅샷이 안 끝나면 기다리다가,
        // 그마저 늦으면 여유 시간 뒤에 강제로 정리합니다.
        setTimeout(() => {
          try {
            globalThis.전송진단.종료신호(계측기, 전송결과.성공 ? "SEND-OK" : 전송결과.코드);
          } catch (e) {
            /* 무시 */
          }
        }, 6000);
      ...
      .catch((e) => {
        // 예상 못 한 오류로도 잠금이 남지 않게 합니다.
        try {
          응답({ 성공: false, 사유: "전송 중 오류: " + (e && e.message) });
        } catch (_) {
          /* 이미 응답했으면 무시 */
        }
      })
